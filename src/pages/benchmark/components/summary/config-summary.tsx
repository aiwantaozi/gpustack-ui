...
import { DatasetValueMap, loadTypeOptions } from '../../config';
...
// v2.1: a compact one-line config summary at the top of Summary. Full config
...
const ConfigSummary: React.FC = () => {
  ...
  // Custom-dataset label: prefer the creation-time snapshot (self-contained,
  // survives resource deletion), then resolve dataset_id against loaded
  // resources, then the bare type. Random / ShareGPT show their type string.
  const isCustomDataset =
    detailData?.dataset_name === DatasetValueMap.Custom ||
    detailData?.dataset_id != null;
  let datasetLabel: string | undefined = detailData?.dataset_name;
  if (isCustomDataset) {
    const snap = detailData?.snapshot?.dataset;
    const snapLabel = snap ? getDatasetSourceLabel(snap) : '';
    const found = (datasetResources || []).find(
      (d: any) => d.id === detailData?.dataset_id
    );
    datasetLabel =
      snapLabel ||
      (found && getDatasetSourceLabel(found)) ||
